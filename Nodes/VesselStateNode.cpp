...
VesselStateNode::VesselStateNode(MessageBus& msgBus)
	: ActiveNode(NodeID::VesselState, msgBus),
		...
		m_rudderFeedback(0),m_sailFeedback(0),m_ArduinoBattery(0)
{
	msgBus.registerNode(this, MessageType::CompassData);
	msgBus.registerNode(this, MessageType::GPSData);
	msgBus.registerNode(this, MessageType::WindData);
	msgBus.registerNode(this, MessageType::ArduinoData);
	msgBus.registerNode(this, MessageType::ActuatorFeedback);
}
...
void VesselStateNode::processMessage(const Message* msg)
{
	...
	case MessageType::ActuatorFeedback:
		processActuatorMessage((ActuatorPositionMsg*)msg);
		break;
	default:
		return;
	}
...
void VesselStateNode::processActuatorMessage(ActuatorPositionMsg* msg)
{
	m_rudderFeedback = msg->rudderPosition();
	m_sailFeedback = msg->sailPosition();
}
...
void VesselStateNode::VesselStateThreadFunc(void* nodePtr)
{
	...
	while(true)
	{
		...
		VesselStateMsg* vesselState = new VesselStateMsg(	node->m_CompassHeading, node->m_CompassPitch,
															...
															node->m_WindTemp, node->m_ArduinoPressure, node->m_rudderFeedback,
															node->m_sailFeedback, node->m_ArduinoBattery);
