...
{
	// Multiple the value from microseconds to quarter-microseconds which the maestro expects
	if(not MaestroController::writeCommand(MaestroCommands::SetPosition, m_Channel, value * 4))
	{
		Logger::error("%s:%d Actuator: %d Failed to write position command", __FILE__, __LINE__, (int)nodeID());
	}
}
