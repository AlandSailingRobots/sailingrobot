...
#define DECL_GSSVX(PREFIX, FLOATTYPE, KEYTYPE)                                                   \
    ...
    inline float SuperLU_gssvx(superlu_options_t* options, SuperMatrix* A, int* perm_c,          \
        ...
        PREFIX##gssvx(options, A, perm_c, perm_r, etree, equed, R, C, L, U, work, lwork, B, X,   \
                      recip_pivot_growth, rcond, ferr, berr, &mem_usage, stats, info);           \
...
#define DECL_GSISX(PREFIX, FLOATTYPE, KEYTYPE)                                                   \
    ...
    inline float SuperLU_gsisx(superlu_options_t* options, SuperMatrix* A, int* perm_c,          \
                               int* perm_r, int* etree, char* equed, FLOATTYPE* R, FLOATTYPE* C, \
                               SuperMatrix* L, SuperMatrix* U, void* work, int lwork,            \
                               SuperMatrix* B, SuperMatrix* X, FLOATTYPE* recip_pivot_growth,    \
                               FLOATTYPE* rcond, SuperLUStat_t* stats, int* info, KEYTYPE) {     \
        PREFIX##mem_usage_t mem_usage;                                                           \
        PREFIX##gsisx(options, A, perm_c, perm_r, etree, equed, R, C, L, U, work, lwork, B, X,   \
                      recip_pivot_growth, rcond, &mem_usage, stats, info);                       \
        return mem_usage.for_lu; /* bytes used by the factor storage */                          \
    }
...
MappedSparseMatrix<Scalar, Flags, Index> map_superlu(SluMatrix& sluMat) {
    ...
    return MappedSparseMatrix<Scalar, Flags, Index>(
        sluMat.nrow, sluMat.ncol, sluMat.storage.outerInd[outerSize], sluMat.storage.outerInd,
        sluMat.storage.innerInd, reinterpret_cast<Scalar*>(sluMat.storage.values));
}
...
class SuperLUBase : internal::noncopyable {
   ...
   public:
     ...
     */
    template <typename Rhs>
    inline const internal::sparse_solve_retval<SuperLUBase, Rhs> solve(
 ...
 * using the SuperLU library. The sparse matrix A must be squared and invertible. The vectors or
 * matrices X and B can be either dense or sparse.
 *
...
void SuperLU<MatrixType>::factorize(const MatrixType& a) {
    ...
    StatInit(&m_sluStat);
    SuperLU_gssvx(&m_sluOptions, &m_sluA, m_q.data(), m_p.data(), &m_sluEtree[0], &m_sluEqued,
                  &m_sluRscale[0], &m_sluCscale[0], &m_sluL, &m_sluU, NULL, 0, &m_sluB, &m_sluX,
                  &recip_pivot_growth, &rcond, &ferr, &berr, &m_sluStat, &info, Scalar());
    StatFree(&m_sluStat);
...
void SuperLU<MatrixType>::_solve(const MatrixBase<Rhs>& b, MatrixBase<Dest>& x) const {
    ...
    SuperLU_gssvx(&m_sluOptions, &m_sluA, m_q.data(), m_p.data(), &m_sluEtree[0], &m_sluEqued,
                  &m_sluRscale[0], &m_sluCscale[0], &m_sluL, &m_sluU, NULL, 0, &m_sluB, &m_sluX,
                  &recip_pivot_growth, &rcond, &m_sluFerr[0], &m_sluBerr[0], &m_sluStat, &info,
                  Scalar());
    StatFree(&m_sluStat);
    m_info = info == 0 ? Success : NumericalIssue;
}
...
void SuperLUBase<MatrixType, Derived>::extractData() const {
    eigen_assert(m_factorizationIsOk &&
                 "The decomposition is not in a valid state for extracting factors, you must first "
...
typename SuperLU<MatrixType>::Scalar SuperLU<MatrixType>::determinant() const {
    eigen_assert(m_factorizationIsOk &&
                 "The decomposition is not in a valid state for computing the determinant, you "
 ...
 *
 * This class allows to solve for an approximate solution of A.X = B sparse linear problems via an
 * incomplete LU factorization using the SuperLU library. This class is aimed to be used as a
 * preconditioner of the iterative linear solvers.
 *
...
void SuperILU<MatrixType>::factorize(const MatrixType& a) {
    ...
    StatInit(&m_sluStat);
    SuperLU_gsisx(&m_sluOptions, &m_sluA, m_q.data(), m_p.data(), &m_sluEtree[0], &m_sluEqued,
                  &m_sluRscale[0], &m_sluCscale[0], &m_sluL, &m_sluU, NULL, 0, &m_sluB, &m_sluX,
                  &recip_pivot_growth, &rcond, &m_sluStat, &info, Scalar());
    StatFree(&m_sluStat);
