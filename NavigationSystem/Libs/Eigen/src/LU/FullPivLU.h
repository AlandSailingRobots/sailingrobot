 ...
 *
 * This decomposition provides the generic approach to solving systems of linear equations,
 * computing the rank, invertibility, inverse, kernel, and determinant.
 *
 * This LU decomposition is very stable and well tested with large matrices. However there are use
 * cases where the SVD decomposition is inherently more stable and/or flexible. For example, when
 * computing the kernel of a matrix, working with the SVD allows to select the smallest singular
...
class FullPivLU {
   public:
    ...
    /** \returns the kernel of the matrix, also called its null-space. The columns of the returned
     * matrix will form a basis of the kernel.
     *
     * \note If the kernel has dimension zero, then the returned matrix is a column-vector filled
     * with zeros.
     *
    ...
    /** \returns the image of the matrix, also called its column-space. The columns of the returned
     * matrix will form a basis of the kernel.
     *
     * \param originalMatrix the original matrix, of which *this is the LU decomposition.
     *                       The reason why it is needed to pass it here, is that this allows
     *                       a large optimization, as otherwise this method would need to
     * reconstruct it from the LU decomposition.
     *
     * \note If the image has dimension zero, then the returned matrix is a column-vector filled
     * with zeros.
     *
     * \note This method has to determine which pivots should be considered nonzero.
     *       For that, it uses the threshold value that you can control by calling
    ...
    inline const internal::solve_retval<FullPivLU, Rhs> solve(const MatrixBase<Rhs>& b) const {
        eigen_assert(m_isInitialized && "LU is not initialized.");
     ...
     */
    inline const internal::solve_retval<FullPivLU, typename MatrixType::IdentityReturnType>
    inverse() const {
        eigen_assert(m_isInitialized && "LU is not initialized.");
...
FullPivLU<MatrixType>& FullPivLU<MatrixType>::compute(const MatrixType& matrix) {
    ...
    eigen_assert(matrix.rows() <= NumTraits<int>::highest() &&
                 matrix.cols() <= NumTraits<int>::highest());
...
struct kernel_retval<FullPivLU<_MatrixType>> : kernel_retval_base<FullPivLU<_MatrixType>> {
    ...
    enum {
        MaxSmallDimAtCompileTime = EIGEN_SIZE_MIN_PREFER_FIXED(MatrixType::MaxColsAtCompileTime,
    ...
    void evalTo(Dest& dst) const {
        ...
        // negative of the RHS, but for performance we just put the negative sign elsewhere, see
        // below.
...
struct image_retval<FullPivLU<_MatrixType>> : image_retval_base<FullPivLU<_MatrixType>> {
    ...
    void evalTo(Dest& dst) const {
        ...
        for (Index i = 0; i < rank(); ++i)
            dst.col(i) =
                originalMatrix().col(dec().permutationQ().indices().coeff(pivots.coeff(i)));
    }
};
...
struct solve_retval<FullPivLU<_MatrixType>, Rhs> : solve_retval_base<FullPivLU<_MatrixType>, Rhs> {
    ...
    void evalTo(Dest& dst) const {
        ...
        const Index rows = dec().rows(), cols = dec().cols(), nonzero_pivots = dec().rank();
        ...
        if (rows > cols) {
            c.bottomRows(rows - cols) -= dec().matrixLU().bottomRows(rows - cols) * c.topRows(cols);
        }
...
inline const FullPivLU<typename MatrixBase<Derived>::PlainObject> MatrixBase<Derived>::fullPivLu()
    const {
