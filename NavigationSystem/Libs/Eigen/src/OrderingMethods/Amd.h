 ...
 * The input matrix \a C must be a selfadjoint compressed column major SparseMatrix object. Both the
 * upper and lower parts have to be stored, but the diagonal entries are optional. On exit the
 * values of C are destroyed */
template <typename Scalar, typename Index>
void minimum_degree_ordering(SparseMatrix<Scalar, ColMajor, Index>& C,
                             PermutationMatrix<Dynamic, Dynamic, Index>& perm) {
    using std::sqrt;

    int d, dk, dext, lemax = 0, e, elenk, eln, i, j, k, k1, k2, k3, jlast, ln, dense, nzmax,
                     mindeg = 0, nvi, nvj, nvk, mark, wnvi, ok, nel = 0, p, p1, p2, p3, p4, pj, pk,
                     pk1, pk2, pn, q, t;
    unsigned int h;
    ...
    for (i = 0; i < n; i++) {
        ...
        if (d == 1 && has_diag) /* node i is empty */
        {
            ...
            w[i] = 0;
                   ...
                   !has_diag) /* node i is dense or has no structural diagonal element */
        {
    ...
    {
        if (Cp[i] == -1)
            k = internal::cs_tdfs<Index>(i, k, head, next, perm.indices().data(), w);
    }
