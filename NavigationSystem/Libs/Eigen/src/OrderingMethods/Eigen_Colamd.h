...
template <typename Index>
static Index init_rows_cols(Index n_row,
                            Index n_col,
...
template <typename Index>
static void init_scoring(Index n_row,
                         Index n_col,
                         Colamd_Row<Index> Row[],
                         colamd_col<Index> Col[],
                         Index A[],
...
template <typename Index>
static Index find_ordering(Index n_row,
                           Index n_col,
                           Index Alen,
                           Colamd_Row<Index> Row[],
                           colamd_col<Index> Col[],
...
template <typename Index>
static void detect_super_cols(colamd_col<Index> Col[],
                              Index A[],
                              Index head[],
                              Index row_start,
                              Index row_length);
...
template <typename Index>
static Index garbage_collection(Index n_row,
                                Index n_col,
                                Colamd_Row<Index> Row[],
                                colamd_col<Index> Col[],
                                Index A[],
 ...
 */
template <typename Index>
static bool colamd(Index n_row,
                   Index n_col,
                   Index Alen,
                   Index* A,
                   Index* p,
    ...
    ngarbage = Eigen::internal::find_ordering(n_row, n_col, Alen, Row, Col, A, p, n_col2, max_deg,
                                              2 * nnz);
...
static void detect_super_cols(
    ...
    while (rp < rp_end) {
        ...
        for (super_c = first_col; super_c != COLAMD_EMPTY;
            ...
            for (c = Col[super_c].shared4.hash_next; c != COLAMD_EMPTY;
