...
struct sparse_solve_retval_base
   ...
   protected:
    ...
    inline void defaultEvalTo(SparseMatrix<DestScalar, DestOptions, DestIndex>& dst) const {
        // we process the sparse rhs per block of NbColsAtOnce columns temporarily stored into a
        // dense matrix.
...
#define EIGEN_MAKE_SPARSE_SOLVE_HELPERS(DecompositionType, Rhs)                     \
    ...
    using Base::rows;                                                               \
    using Base::cols;                                                               \
    sparse_solve_retval(const DecompositionType& dec, const Rhs& rhs) : Base(dec, rhs) {}
