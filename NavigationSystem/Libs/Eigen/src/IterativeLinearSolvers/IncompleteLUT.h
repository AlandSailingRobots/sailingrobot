...
class IncompleteLUT : internal::noncopyable {
   ...
   public:
    ...
    inline const internal::solve_retval<IncompleteLUT, Rhs> solve(const MatrixBase<Rhs>& b) const {
        eigen_assert(m_isInitialized && "IncompleteLUT is not initialized.");
        eigen_assert(
            cols() == b.rows() &&
 ...
 * \param fillfactor  This is used to compute the  number @p fill_in of largest elements to keep on
 *each row.
 **/
template <typename Scalar>
...
void IncompleteLUT<Scalar>::factorize(const _MatrixType& amat) {
    ...
    VectorXi jr(n);  // Indicate the position of the nonzero elements in the vector u -- A zero
