...
struct traits<Minor<MatrixType>> : traits<MatrixType> {
    ...
    enum {
        ...
        CoeffReadCost = _MatrixTypeNested::CoeffReadCost  // minor is used typically on tiny
                                                          // matrices, where loops are unrolled and
                                                          // the 'if' evaluates at compile time
    };
};
}  // namespace internal
...
class Minor : public MatrixBase<Minor<MatrixType>> {
   public:
    ...
    inline Minor(const MatrixType& matrix, Index row, Index col)
        ...
        eigen_assert(row >= 0 && row < matrix.rows() && col >= 0 && col < matrix.cols());
    }
...
template <typename Derived>
inline const Minor<Derived> MatrixBase<Derived>::minor(Index row, Index col) const {
    return Minor<Derived>(derived(), row, col);
}
