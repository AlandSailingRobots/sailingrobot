...
class Quaternion : public RotationBase<Quaternion<_Scalar>, 3> {
   ...
   public:
    ...
    inline typename internal::cast_return_type<Quaternion, Quaternion<NewScalarType>>::type cast()
        const {
        return
            typename internal::cast_return_type<Quaternion, Quaternion<NewScalarType>>::type(*this);
    }
...
inline Quaternion<Scalar> ei_quaternion_product(const Quaternion<Scalar>& a,
...
template <typename Scalar>
template <typename Derived>
inline typename Quaternion<Scalar>::Vector3 Quaternion<Scalar>::operator*(
    const MatrixBase<Derived>& v) const {
...
template <typename Scalar>
inline typename Quaternion<Scalar>::Matrix3 Quaternion<Scalar>::toRotationMatrix(void) const {
...
/** Sets *this to be a quaternion representing a rotation sending the vector \a a to the vector \a
 * b.
 *
...
template <typename Scalar>
template <typename Derived1, typename Derived2>
inline Quaternion<Scalar>& Quaternion<Scalar>::setFromTwoVectors(const MatrixBase<Derived1>& a,
...
struct ei_quaternion_assign_impl<Other, 4, 1> {
    typedef typename Other::Scalar Scalar;
    static inline void run(Quaternion<Scalar>& q, const Other& vec) { q.coeffs() = vec; }
};
