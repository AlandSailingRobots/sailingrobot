...
class Scaling {
   ...
   public:
    ...
    inline Scaling inverse() const { return Scaling(coeffs().cwise().inverse()); }
    ...
    inline typename internal::cast_return_type<Scaling, Scaling<NewScalarType, Dim>>::type cast()
        const {
        return
            typename internal::cast_return_type<Scaling, Scaling<NewScalarType, Dim>>::type(*this);
    }
    ...
    bool isApprox(const Scaling& other,
...
inline typename Scaling<Scalar, Dim>::TransformType Scaling<Scalar, Dim>::operator*(
