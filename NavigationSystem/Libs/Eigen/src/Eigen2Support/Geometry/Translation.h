...
class Translation {
   ...
   public:
    ...
    inline VectorType operator*(const VectorType& other) const { return m_coeffs + other; }
...
inline typename Translation<Scalar, Dim>::TransformType Translation<Scalar, Dim>::operator*(
    const ScalingType& other) const {
    TransformType res;
    res.matrix().setZero();
...
template <typename Scalar, int Dim>
inline typename Translation<Scalar, Dim>::TransformType Translation<Scalar, Dim>::operator*(
    const LinearMatrixType& linear) const {
    TransformType res;
    res.matrix().setZero();
    res.linear() = linear;
    res.translation() = m_coeffs;
