...
class MappedSuperNodalMatrix {
   ...
   public:
     ...
     */
    Index* colIndexPtr() { return m_nzval_colptr; }

    const Index* colIndexPtr() const { return m_nzval_colptr; }
    ...
    const Index* rowIndex() const { return m_rowind; }
    ...
    const Index* rowIndexPtr() const { return m_rowind_colptr; }
    ...
    const Index* colToSup() const { return m_col_to_sup; }
    /**
     * Return the array of supernode-to-column mapping
     */
    Index* supToCol() { return m_sup_to_col; }

    const Index* supToCol() const { return m_sup_to_col; }
   ...
   protected:
    ...
    Index*
        m_rowind_colptr;  // rowind_colptr[j] stores the location in rowind[] which starts column j
...
/**
 * \brief InnerIterator class to iterate over nonzero values of the current column in the supernodal
 * matrix L
 *
 */
...
void MappedSuperNodalMatrix<Scalar, Index>::solveInPlace(MatrixBase<Dest>& X) const {
    ...
    for (Index k = 0; k <= nsuper(); k++) {
        ...
        Index istart =
            rowIndexPtr()[fsupc];  // Pointer index to the subscript of the current column
