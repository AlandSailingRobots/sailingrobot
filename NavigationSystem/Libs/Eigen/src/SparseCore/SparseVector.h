...
template <typename Dest,
          ...
          int AssignmentKind = !bool(Src::IsVectorAtCompileTime)
                                   ? SVA_RuntimeSwitch
                                   : Src::InnerSizeAtCompileTime == 1 ? SVA_Outer : SVA_Inner>
struct sparse_vector_assign_selector;

}  // namespace internal

template <typename _Scalar, int _Options, typename _Index>
class SparseVector : public SparseMatrixBase<SparseVector<_Scalar, _Options, _Index>> {
   ...
   public:
    ...
    inline SparseVector(const SparseVector& other) : SparseBase(other), m_size(0) {
        check_template_parameters();
        *this = other.derived();
    }

    /** Swaps the values of \c *this and \a other.
     * Overloaded for performance: this version performs a \em shallow swap by swaping pointers and
     * attributes only. \sa SparseMatrixBase::swap()
     */
    ...
    inline SparseVector& operator=(const SparseMatrixBase<OtherDerived>& other) {
        ...
        internal::sparse_vector_assign_selector<SparseVector, OtherDerived>::run(tmp,
                                                                                 other.derived());
        this->swap(tmp);
        return *this;
    }
   ...
   public:
    ...
    // These two functions were here in the 3.1 release, so let's keep them in case some code rely
    // on them.
