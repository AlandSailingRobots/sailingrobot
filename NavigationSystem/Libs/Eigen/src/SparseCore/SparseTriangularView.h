...
struct traits<SparseTriangularView<MatrixType, Mode>> : public traits<MatrixType> {};

}  // namespace internal

template <typename MatrixType, int Mode>
class SparseTriangularView : public SparseMatrixBase<SparseTriangularView<MatrixType, Mode>> {
    enum {
   ...
   public:
    ...
    typename internal::plain_matrix_type_column_major<OtherDerived>::type solve(
...
class SparseTriangularView<MatrixType, Mode>::InnerIterator
   ...
   public:
    EIGEN_STRONG_INLINE InnerIterator(const SparseTriangularView& view, Index outer)
        ...
        if (SkipFirst) {
            while ((*this) &&
                   ((HasUnitDiag || SkipDiag) ? this->index() <= outer : this->index() < outer))
                Base::operator++();
            if (HasUnitDiag)
                m_returnOne = true;
...
template <typename MatrixType, int Mode>
class SparseTriangularView<MatrixType, Mode>::ReverseInnerIterator
...
template <int Mode>
inline const SparseTriangularView<Derived, Mode> SparseMatrixBase<Derived>::triangularView() const {
    return derived();
}
