...
struct traits<SparseSparseProduct<LhsNested, RhsNested>> {
    ...
    enum {
        ...
        Flags = (int(LhsFlags | RhsFlags) & HereditaryBits & RemovedBits) | EvalBeforeAssigningBit |
...
class SparseSparseProduct : internal::no_assignment_operator,
   ...
   public:
    ...
    template <typename Lhs, typename Rhs>
    EIGEN_STRONG_INLINE SparseSparseProduct(const Lhs& lhs,
                                            const Rhs& rhs,
    ...
    SparseSparseProduct pruned(
        const Scalar& reference = 0,
        const RealScalar& epsilon = NumTraits<RealScalar>::dummy_precision()) const {
        using std::abs;
    ...
    void evalTo(Dest& result) const {
        if (m_conservative)
            internal::conservative_sparse_sparse_product_selector<_LhsNested, _RhsNested,
                                                                  Dest>::run(lhs(), rhs(), result);
        else
            internal::sparse_sparse_product_with_pruning_selector<_LhsNested, _RhsNested,
                                                                  Dest>::run(lhs(), rhs(), result,
                                                                             m_tolerance);
    }
   ...
   protected:
    void init() {
        ...
        enum {
            ProductIsValid =
                _LhsNested::ColsAtCompileTime == Dynamic ||
...
inline const typename SparseSparseProductReturnType<Derived, OtherDerived>::Type
    ...
    return typename SparseSparseProductReturnType<Derived, OtherDerived>::Type(derived(),
                                                                               other.derived());
}
