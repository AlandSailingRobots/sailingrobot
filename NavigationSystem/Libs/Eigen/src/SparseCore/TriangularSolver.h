    ...
    static void run(const Lhs& lhs, Rhs& other) {
        for (int col = 0; col < other.cols(); ++col) {
            for (int i = lhs.cols() - 1; i >= 0; --i) {
                ...
                {
                    if (!(Mode & UnitDiag)) {
                        // TODO replace this by a binary search. make sure the binary search is safe
...
void SparseTriangularView<ExpressionType, Mode>::solveInPlace(
    ...
    internal::sparse_solve_triangular_selector<
        ExpressionType, typename internal::remove_reference<OtherCopy>::type, Mode>::run(m_matrix,
                                                                                         otherCopy);
...
struct sparse_solve_triangular_sparse_selector<Lhs, Rhs, Mode, UpLo, ColMajor> {
    ...
    static void run(const Lhs& lhs, Rhs& other) {
        ...
        for (int col = 0; col < other.cols(); ++col) {
            ...
            for (int i = IsLower ? 0 : lhs.cols() - 1; IsLower ? i < lhs.cols() : i >= 0;
                 i += IsLower ? 1 : -1) {
                tempVector.restart();
