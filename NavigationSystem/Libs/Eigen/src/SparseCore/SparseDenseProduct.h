...
struct SparseDenseProductReturnType<Lhs, Rhs, 1> {
    typedef typename internal::conditional<Lhs::IsRowMajor,
...
struct DenseSparseProductReturnType<Lhs, Rhs, 1> {
    typedef typename internal::conditional<Rhs::IsRowMajor,
                                           SparseDenseOuterProduct<Rhs, Lhs, true>,
                                           SparseDenseOuterProduct<Lhs, Rhs, false>>::type Type;
};
...
struct traits<SparseDenseOuterProduct<Lhs, Rhs, Tr>> {
    ...
    enum {
        ...
        RowsAtCompileTime =
            Tr ? int(traits<Rhs>::RowsAtCompileTime) : int(traits<Lhs>::RowsAtCompileTime),
        ColsAtCompileTime =
            Tr ? int(traits<Lhs>::ColsAtCompileTime) : int(traits<Rhs>::ColsAtCompileTime),
        MaxRowsAtCompileTime =
            Tr ? int(traits<Rhs>::MaxRowsAtCompileTime) : int(traits<Lhs>::MaxRowsAtCompileTime),
        MaxColsAtCompileTime =
            Tr ? int(traits<Lhs>::MaxColsAtCompileTime) : int(traits<Rhs>::MaxColsAtCompileTime),
...
template <typename Lhs, typename Rhs, bool Tr>
class SparseDenseOuterProduct : public SparseMatrixBase<SparseDenseOuterProduct<Lhs, Rhs, Tr>> {
   public:
...
class SparseDenseOuterProduct<Lhs, Rhs, Transpose>::InnerIterator
   ...
   public:
    EIGEN_STRONG_INLINE InnerIterator(const SparseDenseOuterProduct& prod, Index outer)
        : Base(prod.lhs(), 0),
          m_outer(outer),
          m_factor(get(prod.rhs(), outer, typename internal::traits<Rhs>::StorageKind())) {}
...
template <typename SparseLhsType,
          ...
          int LhsStorageOrder =
              ((SparseLhsType::Flags & RowMajorBit) == RowMajorBit) ? RowMajor : ColMajor,
          bool ColPerCol =
              ((DenseRhsType::Flags & RowMajorBit) == 0) || DenseRhsType::ColsAtCompileTime == 1>
...
struct sparse_time_dense_product_impl<SparseLhsType, DenseRhsType, DenseResType, RowMajor, true> {
    ...
    static void run(const SparseLhsType& lhs,
...
inline void sparse_time_dense_product(const SparseLhsType& lhs,
    ...
    sparse_time_dense_product_impl<SparseLhsType, DenseRhsType, DenseResType>::run(lhs, rhs, res,
                                                                                   alpha);
}
...
template <typename Lhs, typename Rhs>
class SparseTimeDenseProduct : public ProductBase<SparseTimeDenseProduct<Lhs, Rhs>, Lhs, Rhs> {
   public:
...
template <typename Lhs, typename Rhs>
class DenseTimeSparseProduct : public ProductBase<DenseTimeSparseProduct<Lhs, Rhs>, Lhs, Rhs> {
   public:
