...
class CompressedStorage {
   ...
   public:
    ...
    CompressedStorage(size_t size) : m_values(0), m_indices(0), m_size(0), m_allocatedSize(0) {
        resize(size);
    }
    ...
    /** \returns the largest \c k in [start,end) such that for all \c j in [start,k) index[\c j]\<\a
     * key */
     ...
     * If the value does not exist, then the value \a defaultValue is returned without any
     * insertion. */
    ...
    inline Scalar atInRange(size_t start,
                            size_t end,
                            Index key,
