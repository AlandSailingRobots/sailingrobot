...
inline int umfpack_symbolic(int n_row,
                            ...
                            const double Control[UMFPACK_CONTROL],
                            double Info[UMFPACK_INFO]) {
    return umfpack_zi_symbolic(n_row, n_col, Ap, Ai, &numext::real_ref(Ax[0]), 0, Symbolic, Control,
                               Info);
}
...
inline int umfpack_numeric(const int Ap[],
                           ...
                           const double Control[UMFPACK_CONTROL],
                           double Info[UMFPACK_INFO]) {
    return umfpack_zi_numeric(Ap, Ai, &numext::real_ref(Ax[0]), 0, Symbolic, Numeric, Control,
                              Info);
}
...
inline int umfpack_solve(int sys,
                         ...
                         const double Control[UMFPACK_CONTROL],
                         double Info[UMFPACK_INFO]) {
    return umfpack_zi_solve(sys, Ap, Ai, &numext::real_ref(Ax[0]), 0, &numext::real_ref(X[0]), 0,
...
inline int umfpack_get_lunz(int* lnz,
                            int* unz,
                            int* n_row,
                            int* n_col,
                            int* nz_udiag,
...
inline int umfpack_get_numeric(int Lp[],
    ...
    return umfpack_zi_get_numeric(Lp, Lj, Lx ? &lx0_real : 0, 0, Up, Ui, Ux ? &ux0_real : 0, 0, P,
                                  Q, Dx ? &dx0_real : 0, 0, do_recip, Rs, Numeric);
}

inline int umfpack_get_determinant(double* Mx,
                                   double* Ex,
                                   void* NumericHandle,
...
inline int umfpack_get_determinant(std::complex<double>* Mx,
                                   double* Ex,
                                   void* NumericHandle,
                                   double User_Info[UMFPACK_INFO]) {
...
namespace internal {
template <typename T>
struct umfpack_helper_is_sparse_plain : false_type {};
template <typename Scalar, int Options, typename StorageIndex>
struct umfpack_helper_is_sparse_plain<SparseMatrix<Scalar, Options, StorageIndex>> : true_type {};
template <typename Scalar, int Options, typename StorageIndex>
 ...
 * compressed matrix. \tparam _MatrixType the type of the sparse matrix A, it must be a
 * SparseMatrix<>
 *
...
class UmfPackLU : internal::noncopyable {
   ...
   public:
     ...
     */
    template <typename Rhs>
    inline const internal::sparse_solve_retval<UmfPackLU, Rhs> solve(
     ...
     *
     * The given matrix must has the same sparcity than the matrix on which the pattern anylysis has
     * been performed.
     *
   ...
   protected:
    ...
    void grapInput_impl(const InputMatrixType& mat, internal::true_type) {
        ...
        if (((MatrixType::Flags & RowMajorBit) == RowMajorBit) ||
            sizeof(typename MatrixType::Index) != sizeof(int) || !mat.isCompressed()) {
    ...
    void analyzePattern_impl() {
        int errorCode = 0;
        errorCode = umfpack_symbolic(m_copyMatrix.rows(), m_copyMatrix.cols(), m_outerIndexPtr,
                                     m_innerIndexPtr, m_valuePtr, &m_symbolic, 0, 0);
    ...
    void factorize_impl() {
        int errorCode;
        errorCode = umfpack_numeric(m_outerIndexPtr, m_innerIndexPtr, m_valuePtr, m_symbolic,
                                    &m_numeric, 0, 0);
...
void UmfPackLU<MatrixType>::extractData() const {
    if (m_extractedDataAreDirty) {
        ...
        umfpack_get_numeric(m_l.outerIndexPtr(), m_l.innerIndexPtr(), m_l.valuePtr(),
                            m_u.outerIndexPtr(), m_u.innerIndexPtr(), m_u.valuePtr(), m_p.data(),
...
bool UmfPackLU<MatrixType>::_solve(const MatrixBase<BDerived>& b, MatrixBase<XDerived>& x) const {
    ...
    for (int j = 0; j < rhsCols; ++j) {
        errorCode = umfpack_solve(UMFPACK_A, m_outerIndexPtr, m_innerIndexPtr, m_valuePtr,
                                  &x.col(j).coeffRef(0), &b.const_cast_derived().col(j).coeffRef(0),
                                  m_numeric, 0, 0);
        if (errorCode != 0)
            return false;
    }
...
struct solve_retval<UmfPackLU<_MatrixType>, Rhs> : solve_retval_base<UmfPackLU<_MatrixType>, Rhs> {
