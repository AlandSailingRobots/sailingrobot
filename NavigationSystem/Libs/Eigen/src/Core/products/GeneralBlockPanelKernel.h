 ...
 *
 * \param[in,out] k Input: the third dimension of the product. Output: the blocking size along the
 * same dimension. \param[in,out] m Input: the number of rows of the left hand side. Output: the
 * blocking size along the same dimension. \param[in,out] n Input: the number of columns of the
 * right hand side. Output: the blocking size along the same dimension.
 *
...
void computeProductBlockingSizes(SizeType& k, SizeType& m, SizeType& n) {
    ...
    enum {
        kdiv = KcFactor * 2 * Traits::nr * Traits::RhsProgress * sizeof(RhsScalar),
...
template <typename CJ, typename T>
...
class gebp_traits {
   public:
    ...
    enum {
        ...
        ConjRhs = _ConjRhs,
        Vectorizable =
            packet_traits<LhsScalar>::Vectorizable && packet_traits<RhsScalar>::Vectorizable,
...
class gebp_traits<std::complex<RealScalar>, RealScalar, _ConjLhs, false> {
   public:
    ...
    EIGEN_STRONG_INLINE void madd_impl(const LhsPacket& a,
                                       const RhsPacket& b,
                                       AccPacket& c,
                                       RhsPacket& tmp,
...
class gebp_traits<std::complex<RealScalar>, std::complex<RealScalar>, _ConjLhs, _ConjRhs> {
   public:
    ...
    EIGEN_STRONG_INLINE void unpackRhs(DenseIndex n, const Scalar* rhs, Scalar* b) {
        for (DenseIndex k = 0; k < n; k++) {
            if (Vectorizable) {
                ...
                pstore1<RealPacket>((RealScalar*)&b[k * ResPacketSize * 2 + ResPacketSize],
                                    imag(rhs[k]));
            } else
                b[k] = rhs[k];
        }
    ...
    EIGEN_STRONG_INLINE void acc(const DoublePacket& c,
...
template <typename Scalar,
          ...
          bool PanelMode>
EIGEN_DONT_INLINE void
gemm_pack_lhs<Scalar, Index, Pack1, Pack2, StorageOrder, Conjugate, PanelMode>::operator()(
    Scalar* blockA,
    const Scalar* EIGEN_RESTRICT _lhs,
    Index lhsStride,
    Index depth,
    ...
    eigen_assert((StorageOrder == RowMajor) ||
                 ((Pack1 % PacketSize) == 0 && Pack1 <= 4 * PacketSize));
    ...
    for (Index i = 0; i < peeled_mc; i += Pack1) {
        ...
        if (StorageOrder == ColMajor) {
            for (Index k = 0; k < depth; k++) {
                ...
                if (Pack1 >= 4 * PacketSize)
                    D = ploadu<Packet>(&lhs(i + 3 * PacketSize, k));
                if (Pack1 >= 1 * PacketSize) {
        ...
        if (PanelMode)
            count += Pack1 * (stride - offset - depth);
    }
...
template <typename Scalar, typename Index, int nr, bool Conjugate, bool PanelMode>
EIGEN_DONT_INLINE void gemm_pack_rhs<Scalar, Index, nr, ColMajor, Conjugate, PanelMode>::operator()(
    Scalar* blockB,
    const Scalar* rhs,
    Index rhsStride,
    Index depth,
...
template <typename Scalar, typename Index, int nr, bool Conjugate, bool PanelMode>
EIGEN_DONT_INLINE void gemm_pack_rhs<Scalar, Index, nr, RowMajor, Conjugate, PanelMode>::operator()(
    Scalar* blockB,
    const Scalar* rhs,
    Index rhsStride,
    Index depth,
...
/** \returns the currently set level 1 cpu cache size (in bytes) used to estimate the ideal blocking
 * size parameters. \sa setCpuCacheSize */
...
/** \returns the currently set level 2 cpu cache size (in bytes) used to estimate the ideal blocking
 * size parameters. \sa setCpuCacheSize */
