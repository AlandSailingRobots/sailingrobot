...
template <typename Scalar,
          ...
          bool ConjugateRhs,
          int Version>
EIGEN_DONT_INLINE void selfadjoint_matrix_vector_product<Scalar,
                                                         Index,
                                                         StorageOrder,
                                                         UpLo,
                                                         ConjugateLhs,
                                                         ConjugateRhs,
                                                         Version>::run(Index size,
                                                                       const Scalar* lhs,
                                                                       Index lhsStride,
                                                                       const Scalar* _rhs,
                                                                       Index rhsIncr,
    ...
    // FIXME this copy is now handled outside product_selfadjoint_vector, so it could probably be
    // removed. if the rhs is not sequentially stored in memory we copy it to a temporary buffer,
    // this is because we need to extract packets
    ei_declare_aligned_stack_constructed_variable(Scalar, rhs, size,
                                                  rhsIncr == 1 ? const_cast<Scalar*>(_rhs) : 0);
    if (rhsIncr != 1) {
    ...
    for (Index j = FirstTriangular ? bound : 0; j < (FirstTriangular ? size : bound); j += 2) {
        ...
        // TODO make sure this product is a real * complex and that the rhs is properly conjugated
        // if needed
...
struct SelfadjointProductMatrix<Lhs, LhsMode, false, Rhs, 0, true>
    ...
    void scaleAndAddTo(Dest& dest, const Scalar& alpha) const {
        ...
        internal::gemv_static_vector_if<ResScalar, Dest::SizeAtCompileTime,
            ...
            static_dest;
        internal::gemv_static_vector_if<RhsScalar, _ActualRhsType::SizeAtCompileTime,
                                        _ActualRhsType::MaxSizeAtCompileTime, !UseRhs>
            static_rhs;
...
struct SelfadjointProductMatrix<Lhs, 0, true, Rhs, RhsMode, false>
    ...
    void scaleAndAddTo(Dest& dest, const Scalar& alpha) const {
        ...
        SelfadjointProductMatrix<Transpose<const Rhs>, int(RhsUpLo) == Upper ? Lower : Upper, false,
                                 Transpose<const Lhs>, 0, true>(m_rhs.transpose(),
                                                                m_lhs.transpose())
            .scaleAndAddTo(destT, alpha);
    }
};
