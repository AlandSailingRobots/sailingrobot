...
class MatrixBase : public DenseBase<Derived> {
   public:
     ...
     * guaranteed that the return type of eval() is either PlainObject or const PlainObject&.
     */
    typedef Matrix<typename internal::traits<Derived>::Scalar,
                   internal::traits<Derived>::RowsAtCompileTime,
                   internal::traits<Derived>::ColsAtCompileTime,
                   AutoAlign |
                       (internal::traits<Derived>::Flags & RowMajorBit ? RowMajor : ColMajor),
...
#undef EIGEN_CURRENT_STORAGE_BASE_CLASS
    ...
    template <typename OtherDerived>
    const typename ProductReturnType<Derived, OtherDerived>::Type operator*(
        const MatrixBase<OtherDerived>& other) const;

    template <typename OtherDerived>
    const typename LazyProductReturnType<Derived, OtherDerived>::Type lazyProduct(
        const MatrixBase<OtherDerived>& other) const;
    ...
    const DiagonalProduct<Derived, DiagonalDerived, OnTheRight> operator*(
    ...
    typedef typename internal::add_const<Diagonal<const Derived, DynamicIndex>>::type
    ...
    template <unsigned int Mode>
    const typename internal::eigen2_part_return_type<Derived, Mode>::type part() const;

    // huuuge hack. make Eigen2's matrix.part<Diagonal>() work in eigen3. Problem: Diagonal is now a
    // class template instead of an integer constant. Solution: overload the part() method template
    // wrt template parameters list.
    ...
    /** \returns true if at least one pair of coefficients of \c *this and \a other are not exactly
     * equal to each other. \warning When using floating point scalar values you probably should
     * rather use a fuzzy comparison such as isApprox() \sa isApprox(), operator== */
    template <typename OtherDerived>
    inline bool operator!=(const MatrixBase<OtherDerived>& other) const {
    ...
    struct cross_product_return_type {
        typedef typename internal::scalar_product_traits<
            typename internal::traits<Derived>::Scalar,
    ...
    template <typename OtherDerived>
    typename cross_product_return_type<OtherDerived>::type cross(
        const MatrixBase<OtherDerived>& other) const;
    template <typename OtherDerived>
    ...
    template <typename ProductDerived, typename Lhs, typename Rhs>
    Derived& operator+=(
        const Flagged<ProductBase<ProductDerived, Lhs, Rhs>, 0, EvalBeforeAssigningBit>& other);
    ...
    template <int Size>
    VectorBlock<Derived, Size> start();
    template <int Size>
    const VectorBlock<const Derived, Size> start() const;
    template <int Size>
    VectorBlock<Derived, Size> end();
    template <int Size>
    const VectorBlock<const Derived, Size> end() const;
