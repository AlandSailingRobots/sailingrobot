...
struct traits<ReturnByValue<Derived>> : public traits<typename traits<Derived>::ReturnType> {
    enum {
        ...
        Flags = (traits<typename traits<Derived>::ReturnType>::Flags | EvalBeforeNestingBit) &
                ~DirectAccessBit
    };
};

/* The ReturnByValue object doesn't even have a coeff() method.
 * So the only way that nesting it in an expression can work, is by evaluating it into a plain
 * matrix. So internal::nested always gives the plain return matrix type.
 *
...
template <typename Derived>
class ReturnByValue : internal::no_assignment_operator,
