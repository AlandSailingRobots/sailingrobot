...
struct scalar_product_op {
    enum {
        ...
        Vectorizable = is_same<LhsScalar, RhsScalar>::value && packet_traits<LhsScalar>::HasMul &&
...
struct functor_traits<scalar_min_op<Scalar>> {
    enum { Cost = NumTraits<Scalar>::AddCost, PacketAccess = packet_traits<Scalar>::HasMin };
};
...
struct functor_traits<scalar_max_op<Scalar>> {
    enum { Cost = NumTraits<Scalar>::AddCost, PacketAccess = packet_traits<Scalar>::HasMax };
};
...
struct functor_traits<scalar_difference_op<Scalar>> {
    enum { Cost = NumTraits<Scalar>::AddCost, PacketAccess = packet_traits<Scalar>::HasSub };
};
...
struct scalar_quotient_op {
    enum {
        // TODO vectorize mixed product
        Vectorizable = is_same<LhsScalar, RhsScalar>::value && packet_traits<LhsScalar>::HasDiv &&
...
struct scalar_cmp_op<Scalar, cmp_UNORD> {
    EIGEN_EMPTY_STRUCT_CTOR(scalar_cmp_op)
    EIGEN_STRONG_INLINE bool operator()(const Scalar& a, const Scalar& b) const {
        return !(a <= b || b <= a);
    }
};
template <typename Scalar>
...
struct linspaced_op_impl<Scalar, false> {
    ...
    linspaced_op_impl(const Scalar& low, const Scalar& step)
        : m_low(low),
          ...
          m_base(padd(pset1<Packet>(low),
                      pmul(pset1<Packet>(step), plset<Scalar>(-packet_traits<Scalar>::size)))) {}
...
struct linspaced_op_impl<Scalar, true> {
    ...
    linspaced_op_impl(const Scalar& low, const Scalar& step)
        : m_low(low),
          ...
          m_stepPacket(pset1<Packet>(m_step)),
    ...
    EIGEN_STRONG_INLINE const Packet packetOp(Index i) const {
        return internal::padd(m_lowPacket,
                              pmul(m_stepPacket, padd(pset1<Packet>(Scalar(i)), m_interPacket)));
    }
...
// nested expressions).
template <typename Scalar, bool RandomAccess = true>
struct linspaced_op;
template <typename Scalar, bool RandomAccess>
...
struct linspaced_op {
    ...
    linspaced_op(const Scalar& low, const Scalar& high, DenseIndex num_steps)
        : impl((num_steps == 1 ? high : low),
               (num_steps == 1 ? Scalar() : (high - low) / Scalar(num_steps - 1))) {}
...
// In Eigen, any binary op (Product, CwiseBinaryOp) require the Lhs and Rhs to have the same scalar
// type, except for multiplication where the mixing of different types is handled by
// scalar_product_traits In particular, real * complex<real> is allowed.
// FIXME move this to functor_traits adding a functor_default
template <typename Functor>
...
struct functor_traits<scalar_sqrt_op<Scalar>> {
    enum { Cost = 5 * NumTraits<Scalar>::MulCost, PacketAccess = packet_traits<Scalar>::HasSqrt };
};
...
// this macro is really needed because functor_traits must be specialized after it is declared but
// before it is used...
