...
inline typename packet_traits<Scalar>::type plset(const Scalar& a) {
    return a;
}
...
template <typename Packet>
inline Packet preduxp(const Packet* vecs) {
    return vecs[0];
}
...
/** \internal copy a packet with constant coeficient \a a (e.g., [a,a,a,a]) to \a *to. \a to must be
 * 16 bytes aligned */
// NOTE: this function must really be templated on the packet type (think about different packet
// types for the same scalar type)
template <typename Packet>
...
/** \internal update \a first using the concatenation of the packet_size minus \a Offset last
 * elements of \a first and \a Offset first elements of \a second.
 *
