 ...
 *
 * How it's used:
 * To allow to defined the global math functions (like sin...) in certain cases, like the Array
 * expressions. When you do sin(array1+array2), the object array1+array2 has a complicated
 * expression type, all what you want to know is that it inherits ArrayBase. So we implement a
 * partial specialization of sin_impl for ArrayBase<Derived>. So we must make sure to use
 * sin_impl<ArrayBase<Derived> > and not sin_impl<Derived>, otherwise our partial specialization
 * won't be used. How does sin know that? That's exactly what global_math_functions_filtering_base
 * tells it.
 *
 * How it's implemented:
 * SFINAE in the style of enable_if. Highly susceptible of breaking compilers. With GCC, it sure
 * does work, but if you replace the typename dummy by an integer template parameter, it doesn't
 * work anymore!
 */
...
template <typename Scalar>
struct random_impl
    : random_default_impl<Scalar, NumTraits<Scalar>::IsComplex, NumTraits<Scalar>::IsInteger> {};
...
struct scalar_fuzzy_default_impl<Scalar, false, false> {
    ...
    static inline bool isMuchSmallerThan(const Scalar& x,
                                         const OtherScalar& y,
...
template <typename Scalar>
struct scalar_fuzzy_impl : scalar_fuzzy_default_impl<Scalar,
