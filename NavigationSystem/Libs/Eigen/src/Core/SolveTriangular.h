   ...
   private:
    enum {
        RhsIsVectorAtCompileTime =
            (Side == OnTheLeft ? Rhs::ColsAtCompileTime : Rhs::RowsAtCompileTime) == 1
    };

   public:
    enum {
        Unrolling = (RhsIsVectorAtCompileTime && Rhs::SizeAtCompileTime != Dynamic &&
...
struct triangular_solver_selector<Lhs, Rhs, Side, Mode, NoUnrolling, 1> {
    ...
    static void run(const Lhs& lhs, Rhs& rhs) {
        ...
        triangular_solve_vector<
            LhsScalar, RhsScalar, typename Lhs::Index, Side, Mode,
...
struct triangular_solver_selector<Lhs, Rhs, Side, Mode, NoUnrolling, Dynamic> {
    ...
    static void run(const Lhs& lhs, Rhs& rhs) {
        ...
        triangular_solve_matrix<
            ...
            (Rhs::Flags & RowMajorBit) ? RowMajor : ColMajor>::run(size, othersize,
...
struct triangular_solver_unroller<Lhs, Rhs, Mode, Index, Size, false> {
    ...
    static void run(const Lhs& lhs, Rhs& rhs) {
        if (Index > 0)
            rhs.coeffRef(RowIndex) -= lhs.row(RowIndex)
                                          .template segment<Index>(S)
                                          .transpose()
                                          .cwiseProduct(rhs.template segment<Index>(S))
                                          .sum();
...
void TriangularView<MatrixType, Mode>::solveInPlace(const MatrixBase<OtherDerived>& _other) const {
    ...
    eigen_assert(cols() == rows() && ((Side == OnTheLeft && cols() == other.rows()) ||
                                      (Side == OnTheRight && cols() == other.cols())));
    ...
    typedef typename internal::conditional<
        copy, typename internal::plain_matrix_type_column_major<OtherDerived>::type,
        OtherDerived&>::type OtherCopy;
    OtherCopy otherCopy(other);

    internal::triangular_solver_selector<MatrixType,
                                         typename internal::remove_reference<OtherCopy>::type, Side,
                                         Mode>::run(nestedExpression(), otherCopy);
 ...
 *
 * This function returns an expression of the inverse-multiply and can works in-place if it is
 * assigned to the same matrix or vector \a other.
 *
