...
class MapBase<Derived, ReadOnlyAccessors> : public internal::dense_xpr_base<Derived>::type {
   public:
    ...
    typedef typename internal::conditional<bool(internal::is_lvalue<Derived>::value),
                                           Scalar*,
     ...
     *
     * \note When addressing this data, make sure to honor the strides returned by innerStride() and
     * outerStride().
     *
...
#include EIGEN_MAPBASE_PLUGIN
   ...
   protected:
    void checkSanity() const {
        EIGEN_STATIC_ASSERT(
            EIGEN_IMPLIES(internal::traits<Derived>::Flags & PacketAccessBit,
...
class MapBase<Derived, WriteAccessors> : public MapBase<Derived, ReadOnlyAccessors> {
   ...
   public:
    ...
    inline void writePacket(Index index, const PacketScalar& val) {
        EIGEN_STATIC_ASSERT_INDEX_BASED_ACCESS(Derived)
        internal::pstoret<Scalar, PacketScalar, StoreMode>(this->m_data + index * innerStride(),
                                                           val);
    }
