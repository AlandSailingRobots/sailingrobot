...
// When constness must affect traits, it has to be constness on template parameters on which T
// itself depends. For example, traits<Map<const T> > != traits<Map<T> >, but
...
template <typename Derived, int Level = internal::accessors_level<Derived>::value>
...
template <typename _Scalar, int SizeAtCompileTime, int MaxSizeAtCompileTime = SizeAtCompileTime>
...
template <typename Derived,
          int Level = internal::accessors_level<Derived>::has_write_access ? WriteAccessors
                                                                           : ReadOnlyAccessors>
class MapBase;
template <int InnerStrideAtCompileTime, int OuterStrideAtCompileTime>
class Stride;
