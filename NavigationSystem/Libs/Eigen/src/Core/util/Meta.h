 ...
 * If none of these members is provided, then the type of the first argument is returned. FIXME,
 * that behavior is a pretty bad hack.
 */
template <typename T>
struct result_of {};
...
template <int Y,
          ...
          int SupX = ((Y == 1) ? 1 : Y / 2),
          bool Done =
              ((SupX - InfX) <= 1 ? true : ((SupX * SupX <= Y) && ((SupX + 1) * (SupX + 1) > Y)))>
...
/** \internal determines whether the product of two numeric types is allowed and what the return
 * type is */
template <typename T, typename U>
