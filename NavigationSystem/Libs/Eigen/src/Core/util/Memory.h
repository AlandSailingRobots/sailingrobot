...
#if defined(__APPLE__) || defined(_WIN64) || EIGEN_GLIBC_MALLOC_ALREADY_ALIGNED || \
...
#include <unistd.h>
#if ((defined __QNXNTO__) || (defined _GNU_SOURCE) || (defined __PGI) || \
...
inline void* handmade_aligned_malloc(std::size_t size) {
    ...
    void* aligned = reinterpret_cast<void*>(
        (reinterpret_cast<std::size_t>(original) & ~(std::size_t(15))) + 16);
...
inline void check_that_malloc_is_allowed() {
    eigen_assert(is_malloc_allowed() &&
                 "heap allocation is forbidden (EIGEN_RUNTIME_NO_MALLOC is defined and "
                 "g_is_malloc_allowed is false)");
}
#else
inline void check_that_malloc_is_allowed() {}
#endif

/** \internal Allocates \a size bytes. The returned pointer is guaranteed to have 16 bytes
 ...
 */
template <bool Align>
inline void* conditional_aligned_malloc(size_t size) {
    return aligned_malloc(size);
}

template <>
inline void* conditional_aligned_malloc<false>(size_t size) {
    check_that_malloc_is_allowed();
...
template <>
inline void* conditional_aligned_realloc<false>(void* ptr, size_t new_size, size_t) {
 ...
 */
template <typename T>
inline T* construct_elements_of_array(T* ptr, size_t size) {
 ...
 */
template <typename T>
inline void destruct_elements_of_array(T* ptr, size_t size) {
...
/** \internal Allocates \a size objects of type T. The returned pointer is guaranteed to have 16
 * bytes alignment. On allocation error, the returned pointer is undefined, but a std::bad_alloc is
 * thrown. The default constructor of T is called.
 */
template <typename T>
inline T* aligned_new(size_t size) {
...
inline T* conditional_aligned_new(size_t size) {
    check_size_for_overflow<T>(size);
 ...
 */
template <typename T>
inline void aligned_delete(T* ptr, size_t size) {
 ...
 */
template <typename T, bool Align>
inline void conditional_aligned_delete(T* ptr, size_t size) {
    destruct_elements_of_array<T>(ptr, size);
    conditional_aligned_free<Align>(ptr);
}

template <typename T, bool Align>
inline T* conditional_aligned_realloc_new(T* pts, size_t new_size, size_t old_size) {
    check_size_for_overflow<T>(new_size);
    check_size_for_overflow<T>(old_size);
    if (new_size < old_size)
        destruct_elements_of_array(pts + new_size, old_size - new_size);
    T* result = reinterpret_cast<T*>(conditional_aligned_realloc<Align>(
        reinterpret_cast<void*>(pts), sizeof(T) * new_size, sizeof(T) * old_size));
    if (new_size > old_size)
...
template <typename T, bool Align>
inline void conditional_aligned_delete_auto(T* ptr, size_t size) {
    if (NumTraits<T>::RequireInitialization)
        destruct_elements_of_array<T>(ptr, size);
    conditional_aligned_free<Align>(ptr);
}
 ...
 * for example with SSE, "well aligned" means 16-byte-aligned. If vectorization is disabled or if
 * the packet size for the given scalar type is 1, then everything is considered well-aligned.
 *
 * \note If the scalar type is vectorizable, we rely on the following assumptions: sizeof(Scalar) is
 * a power of 2, the packet size in bytes is also a power of 2, and is a multiple of sizeof(Scalar).
 * On the other hand, we do not assume that the array address is a multiple of sizeof(Scalar), as
 * that fails for example with Scalar=double on certain 32-bit platforms, see bug #79.
 *
...
static inline Index first_aligned(const Scalar* array, Index size) {
    ...
    if (PacketSize == 1) {
        // Either there is no vectorization, or a packet consists of exactly 1 scalar so that all
        // elements of the array have the same alignment.
        return 0;
    } else if (size_t(array) & (sizeof(Scalar) - 1)) {
        // There is vectorization for this scalar type, but the array is not aligned to the size of
        // a single scalar. Consequently, no element of the array is well aligned.
        return size;
    } else {
        return std::min<Index>(
            (PacketSize - (Index((size_t(array) / sizeof(Scalar))) & PacketAlignedMask)) &
                PacketAlignedMask,
            size);
    }
}
...
// This helper class construct the allocated memory, and takes care of destructing and freeing the
// handled data at destruction time. In practice this helper class is mainly useful to avoid memory
// leak in case of exceptions.
template <typename T>
class aligned_stack_memory_handler {
   public:
    /* Creates a stack_memory_handler responsible for the buffer \a ptr of size \a size.
     * Note that \a ptr can be 0 regardless of the other parameters.
     * This constructor takes care of constructing/initializing the elements of the buffer if
     *required by the scalar type T (see NumTraits<T>::RequireInitialization). In this case, the
     *buffer elements will also be destructed when this handler will be destructed. Finally, if \a
...
/** \internal
 * Declares, allocates and construct an aligned buffer named NAME of SIZE elements of type TYPE on
 * the stack if SIZE is smaller than EIGEN_STACK_ALLOCATION_LIMIT, and if stack allocation is
 * supported by the platform (currently, this is Linux and Visual Studio only). Otherwise the memory
 * is allocated on the heap. The allocated buffer is automatically deleted when exiting the scope of
 * this declaration. If BUFFER is non null, then the declared variable is simply an alias for
 * BUFFER, and no allocation/deletion occurs. Here is an example: \code
 * {
...
#define EIGEN_ALIGNED_ALLOCA(SIZE)                                                                \
    reinterpret_cast<void*>((reinterpret_cast<size_t>(EIGEN_ALLOCA(SIZE + 16)) & ~(size_t(15))) + \
                            16)
#else
...
#define ei_declare_aligned_stack_constructed_variable(TYPE, NAME, SIZE, BUFFER)                    \
    ...
    TYPE* NAME =                                                                                   \
        (BUFFER) != 0                                                                              \
            ? BUFFER                                                                               \
            : reinterpret_cast<TYPE*>(Eigen::internal::aligned_malloc(sizeof(TYPE) * SIZE));       \
    Eigen::internal::aligned_stack_memory_handler<TYPE> EIGEN_CAT(NAME, _stack_memory_destructor)( \
...
#define EIGEN_MAKE_ALIGNED_OPERATOR_NEW_IF_VECTORIZABLE_FIXED_SIZE(Scalar, Size) \
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW_IF(                                          \
        bool(((Size) != Eigen::Dynamic) && ((sizeof(Scalar) * (Size)) % 16 == 0)))
...
// Case for x64 with PIC. In theory this is only a problem with recent gcc and with medium or large
// code model, not with the default small code model. However, we cannot detect which code model is
// used, and the xchg overhead is negligible anyway.
#define EIGEN_CPUID(abcd, func, id)                                                    \
    ...
    for (int i = 0; i < 14; ++i) {
        switch (bytes[i]) {
            ...
            case 0x49:
                ...
                break;  // code and data L3 cache, 4096 KB, 16 ways, 64 byte lines (P4) or L2 for
                        // core2
            case 0x4A:
                l3 = 6144;
