...
class BandMatrixBase : public EigenBase<Derived> {
   ...
   public:
    ...
    inline Block<CoefficientsType, Dynamic, 1> col(Index i) {
        EIGEN_STATIC_ASSERT((Options & RowMajor) == 0,
    ...
    struct DiagonalIntReturnType {
        enum {
            ReturnOpposite = (Options & SelfAdjoint) &&
                ...
                (RowsAtCompileTime == Dynamic || ColsAtCompileTime == Dynamic)
                    ...
                    : (ActualIndex < 0
                           ? EIGEN_SIZE_MIN_PREFER_DYNAMIC(ColsAtCompileTime,
                                                           RowsAtCompileTime + ActualIndex)
                           : EIGEN_SIZE_MIN_PREFER_DYNAMIC(RowsAtCompileTime,
                                                           ColsAtCompileTime - ActualIndex))
        };
    ...
    inline typename DiagonalIntReturnType<N>::Type diagonal() {
        return typename DiagonalIntReturnType<N>::BuildType(coeffs(), supers() - N,
...
struct traits<BandMatrix<_Scalar, _Rows, _Cols, _Supers, _Subs, _Options>> {
    ...
    typedef Matrix<Scalar,
                   DataRowsAtCompileTime,
                   ColsAtCompileTime,
                   Options & RowMajor ? RowMajor : ColMajor>
        CoefficientsType;
};
...
class BandMatrix : public BandMatrixBase<BandMatrix<_Scalar, Rows, Cols, Supers, Subs, Options>> {
   public:
    ...
    inline BandMatrix(Index rows = Rows,
                      Index cols = Cols,
                      Index supers = Supers,
                      Index subs = Subs)
...
class BandMatrixWrapper
   ...
   public:
    ...
    inline BandMatrixWrapper(const CoefficientsType& coeffs,
                             Index rows = _Rows,
                             Index cols = _Cols,
...
class TridiagonalMatrix
    ...
    typedef BandMatrix<Scalar, Size, Size, Options & SelfAdjoint ? 0 : 1, 1, Options | RowMajor>
        Base;
