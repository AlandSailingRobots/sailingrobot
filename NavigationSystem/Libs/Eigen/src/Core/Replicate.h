...
class Replicate
   ...
   public:
    ...
    inline Scalar coeff(Index rowId, Index colId) const {
        ...
        const Index actual_row = internal::traits<MatrixType>::RowsAtCompileTime == 1
                                     ? 0
                                     : RowFactor == 1 ? rowId : rowId % m_matrix.rows();
        const Index actual_col = internal::traits<MatrixType>::ColsAtCompileTime == 1
                                     ? 0
                                     : ColFactor == 1 ? colId : colId % m_matrix.cols();
 ...
 */
template <typename Derived>
const typename DenseBase<Derived>::ReplicateReturnType DenseBase<Derived>::replicate(
    Index rowFactor,
