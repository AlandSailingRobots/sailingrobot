...
struct isApprox_selector {
    static bool run(const Derived& x,
        ...
        return (nested - otherNested).cwiseAbs2().sum() <=
               prec * prec * (min)(nested.cwiseAbs2().sum(), otherNested.cwiseAbs2().sum());
    }
};
...
bool DenseBase<Derived>::isApprox(const DenseBase<OtherDerived>& other,
    ...
    return internal::isApprox_selector<Derived, OtherDerived>::run(derived(), other.derived(),
                                                                   prec);
}
...
bool DenseBase<Derived>::isMuchSmallerThan(const DenseBase<OtherDerived>& other,
                                           const RealScalar& prec) const {
    return internal::isMuchSmallerThan_object_selector<Derived, OtherDerived>::run(
