...
template <typename Derived,
          ...
          bool IsVector =
              bool(Derived::IsVectorAtCompileTime) && bool(OtherDerived::IsVectorAtCompileTime)>
...
template <typename Derived>
struct dense_xpr_base_dispatcher_for_doxygen;  // : public MatrixBase<Derived> {};
...
#endif
{
   ...
   public:
    enum {
        NeedsToAlign =
            SizeAtCompileTime != Dynamic && (internal::traits<Derived>::Flags & AlignedBit) != 0
    };
    ...
    EIGEN_STRONG_INLINE PacketScalar packet(Index rowId, Index colId) const {
        return internal::ploadt<PacketScalar, LoadMode>(
            m_storage.data() + (Flags & RowMajorBit ? colId + rowId * m_storage.cols()
    ...
    /** Resizes the matrix, changing only the number of columns. For the parameter of type
     * NoChange_t, just pass the special value \c NoChange as in the example below.
     *
     ...
     */
    inline void resize(NoChange_t, Index nbCols) { resize(rows(), nbCols); }

    /** Resizes the matrix, changing only the number of rows. For the parameter of type NoChange_t,
     * just pass the special value \c NoChange as in the example below.
     *
     ...
     * \sa resize(Index,Index)
     */
    inline void resize(Index nbRows, NoChange_t) { resize(nbRows, cols()); }
    ...
    EIGEN_STRONG_INLINE void resizeLike(const EigenBase<OtherDerived>& _other) {
        ...
        internal::check_rows_cols_for_overflow<MaxSizeAtCompileTime>::run(other.rows(),
                                                                          other.cols());
    ...
    PlainObjectBase(PlainObjectBase&& other) : m_storage(std::move(other.m_storage)) {}
    ...
    /** Copy constructor */
    EIGEN_STRONG_INLINE PlainObjectBase(const PlainObjectBase& other) : m_storage() {
        _check_template_params();
        lazyAssign(other);
    }

    template <typename OtherDerived>
    EIGEN_STRONG_INLINE PlainObjectBase(const DenseBase<OtherDerived>& other) : m_storage() {
        _check_template_params();
        lazyAssign(other);
    }
    ...
    EIGEN_STRONG_INLINE PlainObjectBase(const EigenBase<OtherDerived>& other)
        : m_storage(other.derived().rows() * other.derived().cols(),
                    other.derived().rows(),
                    other.derived().cols()) {
        _check_template_params();
        internal::check_rows_cols_for_overflow<MaxSizeAtCompileTime>::run(other.derived().rows(),
                                                                          other.derived().cols());
        Base::operator=(other.derived());
    }
     ...
     * unaligned Map objects, while the AlignedMap() functions return aligned Map objects and thus
     * should be called only with 16-byte-aligned \a data pointers.
     *
     * \see class Map
     */
    //@{
    ...
    template <int Outer, int Inner>
    static inline typename StridedMapType<Stride<Outer, Inner>>::type Map(
        Scalar* data,
        const Stride<Outer, Inner>& stride) {
    ...
    MapAligned(const Scalar* data, Index rows, Index cols, const Stride<Outer, Inner>& stride) {
        return typename StridedConstAlignedMapType<Stride<Outer, Inner>>::type(data, rows, cols,
                                                                               stride);
    }
    template <int Outer, int Inner>
    static inline typename StridedAlignedMapType<Stride<Outer, Inner>>::type
...
#include EIGEN_PLAINOBJECTBASE_PLUGIN
   ...
   protected:
    ...
    /** \internal Like _set() but additionally makes the assumption that no aliasing effect can
     * happen (which is the case when creating a new matrix) so one can enforce lazy evaluation.
     *
    ...
    EIGEN_STRONG_INLINE Derived& _set_noalias(const DenseBase<OtherDerived>& other) {
        ...
        return internal::assign_selector<Derived, OtherDerived, false>::run(this->derived(),
                                                                            other.derived());
    }

    template <typename T0, typename T1>
    EIGEN_STRONG_INLINE void _init2(
        Index nbRows,
        Index nbCols,
    ...
    /** \internal generic implementation of swap for dense storage since for dynamic-sized matrices
    ...
    void _swap(DenseBase<OtherDerived> const& other) {
        ...
        internal::matrix_swap_impl<Derived, OtherDerived, bool(SwapPointers)>::run(
            this->derived(), other.const_cast_derived());
    }
...
struct conservative_resize_like_impl {
    ...
    static void run(DenseBase<Derived>& _this, Index rows, Index cols) {
        ...
        if ((Derived::IsRowMajor &&
             _this.cols() == cols) ||  // row-major and we change only the number of rows
            (!Derived::IsRowMajor &&
    ...
    static void run(DenseBase<Derived>& _this, const DenseBase<OtherDerived>& other) {
        ...
        if ((Derived::IsRowMajor &&
            ...
            (!Derived::IsRowMajor &&
             _this.rows() ==
                 other.rows()))  // column-major and we change only the number of columns
        {
