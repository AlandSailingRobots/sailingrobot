...
void eigen_pastix(pastix_data_t** pastix_data,
                  int pastix_comm,
                  int n,
                  int* ptr,
                  int* idx,
...
void eigen_pastix(pastix_data_t** pastix_data,
    ...
    c_pastix(pastix_data, pastix_comm, n, ptr, idx, reinterpret_cast<COMPLEX*>(vals), perm, invp,
             reinterpret_cast<COMPLEX*>(x), nbrhs, iparm, dparm);
}
...
void eigen_pastix(pastix_data_t** pastix_data,
    ...
    z_pastix(pastix_data, pastix_comm, n, ptr, idx, reinterpret_cast<DCOMPLEX*>(vals), perm, invp,
             reinterpret_cast<DCOMPLEX*>(x), nbrhs, iparm, dparm);
}
...
class PastixBase : internal::noncopyable {
   ...
   public:
    ...
    inline const internal::solve_retval<PastixBase, Rhs> solve(const MatrixBase<Rhs>& b) const {
     ...
     * The statistics related to the different phases of factorization and solve are saved here as
     * well \sa analyzePattern() factorize()
     */
    Array<Index, IPARM_SIZE, 1>& iparm() { return m_iparm; }
     ...
     * well \sa analyzePattern() factorize()
     */
    Array<RealScalar, IPARM_SIZE, 1>& dparm() { return m_dparm; }
     ...
     */
    template <typename Rhs>
    inline const internal::sparse_solve_retval<PastixBase, Rhs> solve(
   ...
   protected:
    ...
    void clean() {
        ...
        internal::eigen_pastix(&m_pastixdata, MPI_COMM_WORLD, 0, 0, 0, (Scalar*)0, m_perm.data(),
                               m_invp.data(), 0, 0, m_iparm.data(), m_dparm.data());
    }
...
void PastixBase<Derived>::init() {
    ...
    internal::eigen_pastix(&m_pastixdata, MPI_COMM_WORLD, 0, 0, 0, (Scalar*)0, 0, 0, 0, 0,
...
void PastixBase<Derived>::analyzePattern(ColSpMatrix& mat) {
    ...
    internal::eigen_pastix(&m_pastixdata, MPI_COMM_WORLD, m_size, mat.outerIndexPtr(),
                           mat.innerIndexPtr(), mat.valuePtr(), m_perm.data(), m_invp.data(), 0, 0,
                           m_iparm.data(), m_dparm.data());
...
bool PastixBase<Base>::_solve(const MatrixBase<Rhs>& b, MatrixBase<Dest>& x) const {
    ...
    for (int i = 0; i < b.cols(); i++) {
        ...
        internal::eigen_pastix(&m_pastixdata, MPI_COMM_WORLD, x.rows(), 0, 0, 0, m_perm.data(),
                               m_invp.data(), &x(0, i), rhs, m_iparm.data(), m_dparm.data());
    }
...
/** \ingroup PaStiXSupport_Module
 * \class PastixLLT
 * \brief A sparse direct supernodal Cholesky (LLT) factorization and solver based on the PaStiX
 * library
 *
 * This class is used to solve the linear systems A.X = B via a LL^T supernodal Cholesky
 * factorization available in the PaStiX library. The matrix A should be symmetric and positive
 * definite WARNING Selfadjoint complex matrices are not supported in the current version of PaStiX
 * The vectors or matrices X and B can be either dense or sparse
 *
 * \tparam MatrixType the type of the sparse matrix A, it must be a SparseMatrix<>
 * \tparam UpLo The part of the matrix to use : Lower or Upper. The default is Lower as required by
 * PaStiX
 *
 * \sa \ref TutorialSparseDirectSolvers
 */
 ...
 * library
 *
 * This class is used to solve the linear systems A.X = B via a LDL^T supernodal Cholesky
 * factorization available in the PaStiX library. The matrix A should be symmetric and positive
 * definite WARNING Selfadjoint complex matrices are not supported in the current version of PaStiX
 * The vectors or matrices X and B can be either dense or sparse
 *
