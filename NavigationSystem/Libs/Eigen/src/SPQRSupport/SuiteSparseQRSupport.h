...
class SPQR {
   ...
   public:
    ...
    void compute(const _MatrixType& matrix) {
        ...
        if (m_useDefaultThreshold) {
            ...
            for (int j = 0; j < mat.cols(); j++)
                max2Norm = (max)(max2Norm, mat.col(j).norm());
            if (max2Norm == RealScalar(0))
                max2Norm = RealScalar(1);
            pivotThreshold =
                20 * (mat.rows() + mat.cols()) * max2Norm * NumTraits<RealScalar>::epsilon();
        }

        cholmod_sparse A;
        A = viewAsCholmod(mat);
        Index col = matrix.cols();
        m_rank = SuiteSparseQR<Scalar>(m_ordering, pivotThreshold, col, &A, &m_cR, &m_E, &m_H,
                                       &m_HPinv, &m_HTau, &m_cc);
    ...
    void _solve(const MatrixBase<Rhs>& b, MatrixBase<Dest>& dest) const {
        ...
        y.topRows(rk) =
            this->matrixR().topLeftCorner(rk, rk).template triangularView<Upper>().solve(
                y2.topRows(rk));
...
struct SPQR_QProduct : ReturnByValue<SPQR_QProduct<SPQRType, Derived>> {
    ...
    void evalTo(ResType& res) const {
        ...
        x_cd = SuiteSparseQR_qmult<Scalar>(method, m_spqr.m_H, m_spqr.m_HTau, m_spqr.m_HPinv, &y_cd,
                                           cc);
...
struct solve_retval<SPQR<_MatrixType>, Rhs> : solve_retval_base<SPQR<_MatrixType>, Rhs> {
