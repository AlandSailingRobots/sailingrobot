...
class QuaternionBase : public RotationBase<Derived, 3> {
   ...
   public:
    ...
    // test_stdvector with GCC 4.4.2. This looks like a GCC bug though, so feel free to re-enable it
    // if it's useful; however notice that we already have the templated operator= above and e.g. in
    // MatrixBase we didn't have to add, in addition to templated operator=, such a non-templated
    // copy operator.
 ...
 * \tparam _Options controls the memory alignment of the coefficients. Can be \# AutoAlign or \#
 * DontAlign. Default is AutoAlign.
 *
 ...
 *
 * \warning Operations interpreting the quaternion as rotation have undefined behavior if the
...
class Quaternion : public QuaternionBase<Quaternion<_Scalar, _Options>> {
   ...
   public:
     ...
     * [\c x, \c y, \c z, \c w]
     */
    inline Quaternion(const Scalar& w, const Scalar& x, const Scalar& y, const Scalar& z)
        : m_coeffs(x, y, z, w) {}
...
struct traits<Map<const Quaternion<_Scalar>, _Options>>
    ...
    typedef traits<
        Quaternion<_Scalar, (int(_Options) & Aligned) == Aligned ? AutoAlign : DontAlign>>
        TraitsBase;
...
class Map<const Quaternion<_Scalar>, _Options>
   ...
   public:
     ...
     * order: \code *coeffs == {x, y, z, w} \endcode
     *
     * If the template parameter _Options is set to #Aligned, then the pointer coeffs must be
     * aligned. */
...
struct quat_product {
    static EIGEN_STRONG_INLINE Quaternion<Scalar> run(const QuaternionBase<Derived1>& a,
...
template <class Derived>
template <class OtherDerived>
EIGEN_STRONG_INLINE Quaternion<typename internal::traits<Derived>::Scalar> QuaternionBase<Derived>::
operator*(const QuaternionBase<OtherDerived>& other) const {
    ...
    return internal::quat_product < Architecture::Target, Derived, OtherDerived,
           ...
           internal::traits<Derived>::IsAligned &&
               internal::traits<OtherDerived>::IsAligned > ::run(*this, other);
}

/** \sa operator*(Quaternion) */
template <class Derived>
template <class OtherDerived>
EIGEN_STRONG_INLINE Derived& QuaternionBase<Derived>::operator*=(
...
template <class Derived>
EIGEN_STRONG_INLINE QuaternionBase<Derived>& QuaternionBase<Derived>::operator=(
...
template <typename Derived1, typename Derived2>
Quaternion<Scalar, Options> Quaternion<Scalar, Options>::FromTwoVectors(
 ...
 */
template <class Derived>
inline Quaternion<typename internal::traits<Derived>::Scalar> QuaternionBase<Derived>::inverse()
    const {
    // FIXME should this function be called multiplicativeInverse and conjugate() be called
    // inverse() or opposite()  ??
 ...
 */
template <class Derived>
inline Quaternion<typename internal::traits<Derived>::Scalar> QuaternionBase<Derived>::conjugate()
    const {
...
struct quaternionbase_assign_impl<Other, 3, 3> {
    ...
    template <class Derived>
    static inline void run(QuaternionBase<Derived>& q, const Other& mat) {
        using std::sqrt;
