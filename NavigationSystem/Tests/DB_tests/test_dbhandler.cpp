...
TEST_CASE("DBhandler")
{
	...
	SECTION("Get/insert wind vane config") {
		DBHandler db("testdb.db");
		db.insert("config_wind_vane", "use_self_steering, wind_sensor_self_steering, self_steering_interval","2,3,0.2");

		REQUIRE(db.retrieveCell("config_wind_vane", "2", "use_self_steering").compare("2") == 0);
		REQUIRE(db.retrieveCell("config_wind_vane", "2", "wind_sensor_self_steering").compare("3") == 0);
		REQUIRE(db.retrieveCell("config_wind_vane", "2", "self_steering_interval").compare("0.2") == 0);
	}
