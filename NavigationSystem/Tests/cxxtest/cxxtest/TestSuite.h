...
struct equals {
    static bool test(X x, Y y) { return (x == y); }
};
...
void doAssertLessThanEquals(const char* file,
    ...
    if (!lessThanEquals<X, Y>::test(x, y)) {
        ...
        tracker().failedAssertLessThanEquals(file, line, xExpr, yExpr, TS_AS_STRING(x),
                                             TS_AS_STRING(y));
        TS_ABORT();
    }
}
...
void doAssertRelation(const char* file,
    ...
    if (!r(x, y)) {
        ...
        tracker().failedAssertRelation(file, line, rExpr, xExpr, yExpr, TS_AS_STRING(x),
                                       TS_AS_STRING(y));
        TS_ABORT();
    }
}
...
void doAssertDelta(const char* file,
    ...
    if (!delta<X, Y, D>::test(x, y, d)) {
        ...
        tracker().failedAssertDelta(file, line, xExpr, yExpr, dExpr, TS_AS_STRING(x),
                                    TS_AS_STRING(y), TS_AS_STRING(d));
        TS_ABORT();
    }
}
...
void doAssertSameFiles(const char* file,
                       int line,
                       const char* file1,
                       const char* file2,
                       const char* message);
...
#define ___TS_ASSERT_SAME_FILES(f, l, x, y, m)               \
    {                                                        \
        _TS_TRY { ___ETS_ASSERT_SAME_FILES(f, l, x, y, m); } \
        __TS_CATCH(f, l)                                     \
    }
...
#define ___TS_ASSERT_THROWS_ASSERT(f, l, e, t, a, m)                               \
    {                                                                              \
        ...
        _TS_CATCH_STD(ex, {                                                        \
            _ts_threw_expected = true;                                             \
            CxxTest::doFailAssertThrows((f), (l), #e, #t, true, (m), ex.what());   \
        })                                                                         \
        _TS_LAST_CATCH({ _ts_threw_else = true; })                                 \
        if (!_ts_threw_expected) {                                                 \
            CxxTest::doFailAssertThrows((f), (l), #e, #t, _ts_threw_else, (m), 0); \
        }                                                                          \
    }
...
#define TSM_ASSERT_THROWS_IS_NAN(m, e, t, x) \
    TSM_ASSERT_THROWS_ASSERT(m, e, t, TSM_ASSERT_IS_NAN(m, x))

#define TS_ASSERT_THROWS_IS_INFINITE(e, t, x) \
    TS_ASSERT_THROWS_ASSERT(e, t, TS_ASSERT_IS_INFINITE(x))
#define TSM_ASSERT_THROWS_IS_INFINITE(m, e, t, x) \
    TSM_ASSERT_THROWS_ASSERT(m, e, t, TSM_ASSERT_IS_INFINITE(m, x))
...
#define ___TS_ASSERT_THROWS_ANYTHING(f, l, e, m)                          \
    {                                                                     \
        ...
        if (!_ts_threw) {                                                 \
            CxxTest::doFailAssertThrows((f), (l), #e, "...", false, (m)); \
        }                                                                 \
    }
