...
int Main(TesterT& tmp, int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if ((CXXTEST_STD(strcmp)(argv[i], "-h") == 0) ||
            (CXXTEST_STD(strcmp)(argv[i], "--help") == 0)) {
            print_help(argv[0]);
            return 0;
            ...
            for (SuiteDescription* sd = RealWorldDescription().firstSuite(); sd; sd = sd->next())
                for (TestDescription* td = sd->firstTest(); td; td = td->next()) {
                    CXXTEST_STD(cout)
                        << td->suiteName() << " " << td->testName() << CXXTEST_STD(endl);
                }
            return 0;
        }
    }
    ...
    if ((argc == 3) && (argv[1][0] != '-')) {
        ...
        if (!status) {
            CXXTEST_STD(cerr) << "ERROR: unknown test '" << argv[1] << "::" << argv[2] << "'"
                              << CXXTEST_STD(endl);
            return -1;
        }
    }
