...
#define TEST_LENGTH 50
        int dotsLeft = TEST_LENGTH - strlen(test.testName());
    ...
    void leaveWorld(const WorldDescription& desc) {
        ...
        newLine();
        (*_o) << "\nFailed " << tracker().failedTests() << " and Skipped "
              << tracker().skippedTests() << " of " << totalTests << endl;
        unsigned numPassed =
            desc.numTotalTests() - tracker().failedTests() - tracker().skippedTests();
    ...
    void warning(const char* file, int line, const char* expression) {
        stop(file, line) << _warningString << ": " << expression << endl;
    }

    void skippedTest(const char* file, int line, const char* expression) {
        if (expression && strlen(expression) > 0)
            stop(file, line) << _warningString << ": Test skipped: " << expression << endl;
    }

    void failedTest(const char* file, int line, const char* expression) {
        stop(file, line) << _errorString << ": Test failed: " << expression << endl;
    }

    void failedAssert(const char* file, int line, const char* expression) {
        stop(file, line) << _errorString << ": Assertion failed: " << expression << endl;
    }
    ...
    void failedAssertEquals(const char* file,
                            ...
                            const char* x,
                            const char* y) {
        stop(file, line) << _errorString << ": Expected (" << xStr << " == " << yStr << "), found ("
                         << x << " != " << y << ")" << endl;
    }
    ...
    void failedAssertSameData(const char* file,
                              ...
                              const void* y,
                              unsigned size) {
        stop(file, line) << _errorString << ": Expected " << sizeStr << " (" << size
    ...
    void failedAssertSameFiles(const char* file,
                               int line,
                               const char*,
                               const char*,
    ...
    void failedAssertDelta(const char* file,
                           ...
                           const char* y,
                           const char* d) {
        stop(file, line) << _errorString << ": Expected (" << xStr << " == " << yStr << ") up to "
                         << dStr << " (" << d << "), found (" << x << " != " << y << ")" << endl;
    }
    ...
    void failedAssertRelation(const char* file,
                              ...
                              const char* x,
                              const char* y) {
        stop(file, line) << _errorString << ": Expected " << relation << "( " << xStr << ", "
                         << yStr << " ), found !" << relation << "( " << x << ", " << y << " )"
                         << endl;
    }
    ...
    void failedAssertPredicate(const char* file,
                               ...
                               const char* xStr,
                               const char* x) {
        stop(file, line) << _errorString << ": Expected " << predicate << "( " << xStr
                         << " ), found !" << predicate << "( " << x << " )" << endl;
    }
    ...
    void failedAssertThrows(const char* file,
                            ...
                            const char* type,
                            bool otherThrown) {
        stop(file, line) << _errorString << ": Expected (" << expression << ") to throw (" << type
                         << ") but it " << (otherThrown ? "threw something else" : "didn't throw")
                         << endl;
    }

    void failedAssertThrowsNot(const char* file, int line, const char* expression) {
        stop(file, line) << _errorString << ": Expected (" << expression
