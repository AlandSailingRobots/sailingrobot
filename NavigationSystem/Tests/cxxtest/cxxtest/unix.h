...
class UNIXErrorFormatter : public TestListener {
   public:
    UNIXErrorFormatter(OutputStream* o, const char* preLine = ":", const char* postLine = "")
        : _reported(false), _o(o), _preLine(preLine), _postLine(postLine) {}
    ...
    void leaveWorld(const WorldDescription& desc) {
        if (tracker().failedTests()) {
            ...
            unsigned numPassed =
                desc.numTotalTests() - tracker().failedTests() - tracer().skippedTests();
            unsigned numTotal = desc.numTotalTests() - tracker().skippedTests();
            if (numTotal == 0) {
                (*_o) << "Success rate: 100%" << endl;
            } else {
                (*_o) << "Success rate: " << (unsigned)(numPassed * 100.0 / numTotal) << "%"
                      << endl;
            }
        }
    }
    ...
    void skippedTest(const char* file, int line, const char* expression) {
        stop(file, line) << "Warning: Test skipped: " << expression << endl;
    }
    ...
    void failedAssertThrows(const char* file,
                            ...
                            const char* type,
                            bool otherThrown) {
        stop(file, line) << "Error: Expected (" << expression << ") to throw (" << type
   ...
   private:
    ...
    void reportTest(void) {
        ...
        (*_o) << tracker().suite().file() << ": In " << tracker().suite().suiteName()
...
class unix : public UNIXErrorFormatter {
   public:
    unix(CXXTEST_STD(ostream) & o = CXXTEST_STD(cerr),
         const char* preLine = ":",
         const char* postLine = "")
