...
class X11Gui : public GuiListener {
   ...
   private:
    ...
    void createWindow() {
        _window =
            XCreateSimpleWindow(_display, RootWindow(_display, 0), 0, 0, 1, 1, 0, 0, _background);
    }
    ...
    void centerWindow() {
        ...
        XMoveResizeWindow(_display, _window, xCenter - (_width / 2), yCenter - (_height / 2),
                          _width, _height);
    }
    ...
    void setWindowName(const char* suiteName, const char* testName) {
        unsigned length =
            strlen(_programName) + strlen(suiteName) + strlen(testName) + sizeof(" - ::()");
