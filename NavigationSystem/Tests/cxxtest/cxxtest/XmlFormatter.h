...
class TeeOutputStreams {
   private:
    class teebuffer : public std::basic_streambuf<char> {
       ...
       public:
        teebuffer(streambuf_t* buf1, streambuf_t* buf2) : buffer1(buf1), buffer2(buf2) {}
...
class ElementInfo {
   public:
    ...
    ElementInfo() : name(), value(), attribute() {}
...
class XmlFormatter : public TestListener {
   public:
    ...
    void enterSuite(const SuiteDescription& desc) {
        ...
        size_t pos = 0;
        while ((pos = classname.find("::", pos)) != CXXTEST_STD(string::npos)) {
    ...
    void enterTest(const TestDescription& desc) {
        ...
        if (stream_redirect)
            CXXTEST_STD(cerr) << "ERROR: The stream_redirect != NULL" << CXXTEST_STD(endl);

        stream_redirect = new TeeOutputStreams(CXXTEST_STD(cout), CXXTEST_STD(cerr));
    }
    ...
    void leaveWorld(const WorldDescription& desc) {
        ...
        (*_o) << "\" tests=\"" << ntests << "\" errors=\"" << nerror << "\" failures=\"" << nfail
              << "\" time=\"" << os.str().c_str() << "\" >";
        _o->endl(*_o);
        (*_o) << _os->str().c_str();
        _os->clear();
    ...
    void failedAssert(const char* file, int line, const char* expression) {
        testFailure(file, line, "failedAssert") << "Assertion failed: " << expression;
    }
    ...
    void failedAssertEquals(const char* file,
        ...
        testFailure(file, line, "failedAssertEquals")
            << "Error: Expected (" << xStr << " == " << yStr << "), found (" << x << " != " << y
            << ")";
    }
    ...
    void failedAssertSameData(const char* file,
        ...
        testFailure(file, line, "failedAssertSameData")
            << "Error: Expected " << sizeStr << " (" << size << ")  bytes to be equal at (" << xStr
            << ") and (" << yStr << "), found";
    }
    ...
    void failedAssertSameFiles(const char* file,
        ...
        testFailure(file, line, "failedAssertSameFiles") << "Error: " << explanation;
    }
    ...
    void failedAssertDelta(const char* file,
        ...
        testFailure(file, line, "failedAssertDelta")
            << "Error: Expected (" << xStr << " == " << yStr << ") up to " << dStr << " (" << d
            << "), found (" << x << " != " << y << ")";
    }
    ...
    void failedAssertDiffers(const char* file,
        ...
        testFailure(file, line, "failedAssertDiffers")
            << "Error: Expected (" << xStr << " != " << yStr << "), found (" << value << ")";
    }
    ...
    void failedAssertRelation(const char* file,
        ...
        testFailure(file, line, "failedAssertRelation")
            << "Error: Expected " << relation << "( " << xStr << ", " << yStr << " ), found !"
            << relation << "( " << x << ", " << y << " )";
    }
    ...
    void failedAssertPredicate(const char* file,
        ...
        testFailure(file, line, "failedAssertPredicate")
            << "Error: Expected " << predicate << "( " << xStr << " ), found !" << predicate << "( "
            << x << " )";
    }
    ...
    void failedAssertThrows(const char* file,
        ...
        testFailure(file, line, "failedAssertThrows")
            << "Error: Expected (" << expression << ") to throw (" << type << ") but it "
   ...
   protected:
    ...
    OutputStream* outputFileStream() const { return _ostr; }
