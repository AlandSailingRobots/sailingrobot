...
class Win32Gui : public GuiListener {
   ...
   private:
    ...
    char _statusTestsDone[sizeof("1000000000 of  (100%)") +
    ...
    void setRatios(unsigned suiteNameRatio,
    ...
    HWND createWindow(LPCTSTR className, DWORD style, HWND parent = (HWND)NULL) {
        return CreateWindow(className, NULL, style, 0, 0, 0, 0, parent, (HMENU)NULL,
                            (HINSTANCE)NULL, (LPVOID)this);
    }
    ...
    void centerMainWindow() {
        ...
        SetWindowPos(_mainWindow, HWND_TOP, xCenter - (windowWidth / 2),
    ...
    void showMainWindow(int mode) { ShowWindow(_mainWindow, mode); }
    ...
    HICON loadStandardIcon(LPCTSTR icon) { return LoadIcon((HINSTANCE)NULL, icon); }

    void setTestCaption(const char* suiteName, const char* testName) {
        setCaption(suiteName, "::", testName, "()");
    }

    void setCaption(const char* a = "",
                    const char* b = "",
                    const char* c = "",
                    const char* d = "") {
        unsigned length = lstrlenA(_title) + sizeof(" - ") + lstrlenA(a) + lstrlenA(b) +
                          lstrlenA(c) + lstrlenA(d);
    ...
    void showSuiteName(const char* suiteName) { setStatusPart(STATUS_SUITE_NAME, suiteName); }

    void showTestName(const char* testName) { setStatusPart(STATUS_TEST_NAME, testName); }

    void showTestsDone() {
        wsprintfA(_statusTestsDone, "%u of %s (%u%%)", _testsDone, _strTotalTests,
    ...
    char* allocate(unsigned length) { return (char*)HeapAlloc(GetProcessHeap(), 0, length); }
