...
void WaypointMgrNode::processVesselStateMessage(StateMessage* msg) {
    ...
    // std::cout << "processVesselStateMessage long: " << m_vesselLongitude << "lat: " <<
    // m_vesselLatitude << std::endl;
}

bool WaypointMgrNode::waypointReached() {
    // double distanceAfterWaypoint = Utility::calculateWaypointsOrthogonalLine(m_nextLongitude,
    // m_nextLatitude, m_prevLongitude,
...
void WaypointMgrNode::sendMessage() {
    ...
    if (m_db.getWaypointValues(m_nextId, m_nextLongitude, m_nextLatitude, m_nextDeclination,
                               m_nextRadius, m_nextStayTime, m_isCheckpoint, m_prevId,
                               m_prevLongitude, m_prevLatitude, m_prevDeclination, m_prevRadius,
                               foundPrev)) {
        if (!foundPrev) {
        ...
        MessagePtr msg = std::make_unique<WaypointDataMsg>(
            m_nextId, m_nextLongitude, m_nextLatitude, m_nextDeclination, m_nextRadius,
            m_nextStayTime, m_isCheckpoint, m_prevId, m_prevLongitude, m_prevLatitude,
            m_prevDeclination, m_prevRadius);
        ...
        Logger::warning("%s No waypoint found, boat is using old waypoint data. No message sent.",
                        __func__);
        m_routeTime.stop();
...
bool WaypointMgrNode::harvestWaypoint() {
    double DistanceToWaypoint =
        CourseMath::calculateDTW(m_vesselLongitude, m_vesselLatitude, m_nextLongitude,
                                 m_nextLatitude);  // Calculate distance to waypoint
    ...
    if (m_nextStayTime >
        0)  // if next waypoint has a time to stay inside its radius, start the timer
    {
        ...
        if (m_waypointTimer.timeReached(
                m_nextStayTime))  // Check if boat has stayed the designated amount of time
        {
...
void WaypointMgrNode::sendNavigationInformation() {
    double distanceToWaypoint = CourseMath::calculateDTW(m_vesselLongitude, m_vesselLatitude,
                                                         m_nextLongitude, m_nextLatitude);
    int16_t bearingToWaypoint = CourseMath::calculateBTW(m_vesselLongitude, m_vesselLatitude,
                                                         m_nextLongitude, m_nextLatitude);

    MessagePtr msg = std::make_unique<CourseDataMsg>(0, distanceToWaypoint, bearingToWaypoint);
    m_MsgBus.sendMessage(std::move(msg));
    // std::cout << "distanceToWaypoint: " << distanceToWaypoint << "  bearingToWaypoint: " <<
    // bearingToWaypoint << std::endl;
}
