...
class StateMessage : public Message {
   public:
    StateMessage(NodeID destinationID,
                 NodeID sourceID,
        ...
        : Message(MessageType::StateMessage, sourceID, destinationID),
          m_VesselHeading(compassHeading),
          m_VesselLat(lat),
          m_VesselLon(lon),
          m_VesselSpeed(gpsSpeed),
          m_VesselCourse(gpsCourse) {}
    ...
    StateMessage(MessageDeserialiser deserialiser) : Message(deserialiser) {
        if (!deserialiser.readFloat(m_VesselHeading) || !deserialiser.readDouble(m_VesselLat) ||
            !deserialiser.readDouble(m_VesselLon) || !deserialiser.readDouble(m_VesselCourse) ||
