...
class ArduinoDataMsg : public Message {
   public:
    ArduinoDataMsg(NodeID destinationID,
                   NodeID sourceID,
                   int pressure,
                   int rudder,
                   int sheet,
                   int battery)
        : Message(MessageType::ArduinoData, sourceID, destinationID),
          m_pressure(pressure),
          m_rudder(rudder),
          m_sheet(sheet),
          m_battery(battery) {}
    ...
    ArduinoDataMsg(int pressure, int rudder, int sheet, int battery)
        : Message(MessageType::ArduinoData, NodeID::None, NodeID::None),
          m_pressure(pressure),
          m_rudder(rudder),
          m_sheet(sheet),
          m_battery(battery) {}

    ArduinoDataMsg(MessageDeserialiser deserialiser) : Message(deserialiser) {
        if (!deserialiser.readInt(m_pressure) || !deserialiser.readInt(m_rudder) ||
            !deserialiser.readInt(m_sheet) || !deserialiser.readInt(m_battery))
