 ...
 *
 * Purpose:
 *		A WaypointDataMsg contains waypoint data such as id, longitude, latitude, declination and
 *radius
 *
 * Developer Notes:
 *
...
class WaypointDataMsg : public Message {
   public:
    WaypointDataMsg(NodeID destinationID,
                    NodeID sourceID,
                    int nextId,
                    ...
                    int prevDeclination,
                    int prevRadius)
        : Message(MessageType::WaypointData, sourceID, destinationID),
          m_nextId(nextId),
          m_nextLongitude(nextLongitude),
          m_nextLatitude(nextLatitude),
          m_nextDeclination(nextDeclination),
          m_nextRadius(nextRadius),
          m_nextStayTime(nextStayTime),
          m_isCheckpoint(isCheckpoint),
          m_prevId(prevId),
          m_prevLongitude(prevLongitude),
          m_prevLatitude(prevLatitude),
          m_prevDeclination(prevDeclination),
          m_prevRadius(prevRadius) {}

    WaypointDataMsg(int nextId,
                    double nextLongitude,
                    double nextLatitude,
                    int nextDeclination,
                    int nextRadius,
                    ...
                    int prevDeclination,
                    int prevRadius)
        : Message(MessageType::WaypointData, NodeID::None, NodeID::None),
          m_nextId(nextId),
          m_nextLongitude(nextLongitude),
          m_nextLatitude(nextLatitude),
          m_nextDeclination(nextDeclination),
    ...
    WaypointDataMsg(MessageDeserialiser deserialiser) : Message(deserialiser) {
        if (!deserialiser.readInt(m_nextId) || !deserialiser.readDouble(m_nextLongitude) ||
            !deserialiser.readDouble(m_nextLatitude) || !deserialiser.readInt(m_nextDeclination) ||
            !deserialiser.readInt(m_nextRadius) || !deserialiser.readInt(m_nextStayTime) ||
            !deserialiser.readBool(m_isCheckpoint) || !deserialiser.readInt(m_prevId) ||
            !deserialiser.readDouble(m_prevLongitude) || !deserialiser.readDouble(m_prevLatitude) ||
            !deserialiser.readInt(m_prevDeclination) || !deserialiser.readInt(m_prevRadius)) {
            m_valid = false;
        }
    }
