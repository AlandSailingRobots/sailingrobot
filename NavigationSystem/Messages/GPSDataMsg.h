...
class GPSDataMsg : public Message {
   public:
    GPSDataMsg(NodeID destinationID,
               NodeID sourceID,
               bool hasFix,
               bool online,
               double lat,
        ...
        : Message(MessageType::GPSData, sourceID, destinationID),
          m_HasFix(hasFix),
          m_Online(online),
          m_Lat(lat),
          m_Lon(lon),
    ...
    GPSDataMsg(bool hasFix,
        ...
        : Message(MessageType::GPSData, NodeID::None, NodeID::None),
          m_HasFix(hasFix),
          m_Online(online),
          m_Lat(lat),
          m_Lon(lon),
    ...
    GPSDataMsg(MessageDeserialiser deserialiser) : Message(deserialiser) {
        uint8_t mode = 0;
        if (!deserialiser.readBool(m_HasFix) || !deserialiser.readBool(m_Online) ||
            !deserialiser.readDouble(m_Lat) || !deserialiser.readDouble(m_Lon) ||
            !deserialiser.readDouble(m_UnixTime) || !deserialiser.readDouble(m_Speed) ||
            !deserialiser.readDouble(m_Course) || !deserialiser.readInt(m_SatCount) ||
