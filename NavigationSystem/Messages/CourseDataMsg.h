...
class CourseDataMsg : public Message {
   public:
    CourseDataMsg(NodeID destinationID,
                  NodeID sourceID,
                  float twd,
                  float distanceToWaypoint,
                  float courseToWaypoint)
        : Message(MessageType::CourseData, sourceID, destinationID),
          m_twd(twd),
          m_dtw(distanceToWaypoint),
          m_ctw(courseToWaypoint) {}

    CourseDataMsg(float twd, float distanceToWaypoint, float courseToWaypoint)
        : Message(MessageType::CourseData, NodeID::None, NodeID::None),
          m_twd(twd),
          m_dtw(distanceToWaypoint),
          m_ctw(courseToWaypoint) {}

    CourseDataMsg(MessageDeserialiser deserialiser) : Message(deserialiser) {
        if (!deserialiser.readFloat(m_twd) || !deserialiser.readFloat(m_dtw) ||
