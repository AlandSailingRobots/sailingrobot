...
printf "$CLR_INFO\nCreating database in $CLR_DIR$INSTALLATION_PATH$REPO_MAIN/$CLR_INFO\n"
	if sqlite3 simu_asr.db < $DIR/simu_createtables.sql;
	...
	printf "$CLR_INFO"
	if sqlite3 simu_asr.db "INSERT INTO server(id, boat_id, boat_pwd, srv_addr) VALUES('1', '$BOATID', '$BOATPWD', '$SRVADDR')";
		then print_result true;
		cp simu_asr.db simu_test_asr.db
